...
async function deployNewToken(signer: SignerProvider, { name, symbol, decimals, supply }: { name: string, symbol: string, decimals: number, supply: number }) {
...
function NewToken() {
  ...
  const form = useForm({
    ...
    validate: {
      ...
      symbol: (value) => /[A-Z]{3,6}/.test(value) ? null : 'Symbol must be 3-6 capital letters',
      decimals: (value) => (value < 0 || value > 18) ? 'Decimals must be between 0 and 18' : null,
      supply: (value) => (value < 0 || !Number.isInteger(value)) ? 'Supply must be positive integer' : null,
    }
  });
  ...
  return (
    <Center h={rem("80%")}>
      ...
      <Box ta="left" w="40rem" mx="auto">
        <TextInput label="Token Name" placeholder="Token Name" {...form.getInputProps('name')} />
