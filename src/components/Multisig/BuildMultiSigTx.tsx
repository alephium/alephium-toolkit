import { Box, Button, Chip, Code, Divider, Grid, Group, Input, Loader, Modal, NumberInput, Paper, Select, SimpleGrid, Stack, Stepper, Text, TextInput, Textarea } from "@mantine/core";
...
import { buildMultisigTx, newMultisigTxStorageKey, useAllMultisig } from "./shared";

function BuildMultisigTx() {
  const form = useForm<{ multisig: string, signers: string[], destinations: { address: string, alphAmount: string }[], signatures: {name: string, signature: string}[], step: number}>({
  ...
  useEffect(() => {
    window.localStorage.setItem(newMultisigTxStorageKey, JSON.stringify(form.values));
  }, [form.values]);
  ...
  return (
    <Box maw={1200} mx="auto">
     ...
     ? <Box maw={900} mx="0" mt="xl" ta="left">
      ...
      <Stack mt="xl">
      <MyBox mx="xl">
      <Text ta='left' fw="700">Select {selectedConfig.mOfN}-of-{selectedConfig.pubkeys.length} Signers</Text>
      <Chip.Group multiple onChange={signers => form.setValues({signers: signers.sort((name0, name1) => {
        const index0 = selectedConfig.pubkeys.findIndex(obj => obj.name === name0)
        const index1 = selectedConfig.pubkeys.findIndex(obj => obj.name === name1)
        ...
        {...selectedConfig.pubkeys.map((signer) => (
          <Chip
            ...
            disabled={form.values.signers.length >= selectedConfig.mOfN && !form.values.signers.includes(signer.name)}
      ...
      <Group mt="lg" position="apart" mx="2rem">
        ...
        <Button color='indigo' onClick={() => { form.setValues({ step: 1 }) }}>
          Build Transaction
        </Button>
      </Group>
      </Stack>}
    ...
    ? <Box maw={800} mx="lg" mt="xl" ta="left">
        ...
        />
        <Group mt="lg" position="apart" mx="2rem">
          <Button onClick={() => { form.setValues({ step: 0 }) }}>
            Back
          </Button>
          <Button onClick={() => { form.setValues({ step: 2 }) }}>
            Copy and Share
          </Button>
        </Group>
      </Box> 
    ...
    ? <Box maw={900} mx="auto" mt="xl" ta="left">
      <MyBox mx="lg">
        <Text ta='left' fw="700">Signatures</Text>
        ...
        <Group mt="lg" position="apart" mx="2rem">
          <Button onClick={() => { form.setValues({ step: 1 }) }}>
            Back
          </Button>
          <Button onClick={() => { form.setValues({ step: 3 }) }}>
            Submit
          </Button>
        </Group>
      </Box>
    : <Box maw={900} mx="auto" mt="xl" ta="left">
      <Group position="center" mt="lg">
        ...
        <Divider mt="xl"/>
        <Group mt="lg" position="apart" mx="2rem">
          <Button color='indigo' onClick={() => { form.setValues({ step: 2 }) }}>
            Back
          </Button>
          <Button component="a" href="https://explorer.alephium.org/" target="_blank">
            Explorer
          </Button>
        </Group>
        </Box> 
    ...
    <Box maw={400} mx="auto" mt="xl" ta="left">
      <Stepper active={form.values.step} onStepClick={s => form.setValues({ step: s })} orientation="vertical">
        <Stepper.Step label="Create" description="Create a new transaction" />
        <Stepper.Step label="Sign" description="Share the transaction to all signers for signature" />
        <Stepper.Step label="Submit" description="Aggregate all signatures and submit the transaction" />
        <Stepper.Step label="Mempool" description="Available on the explorer" />
      </Stepper>
    </Box>
      </Grid.Col>
